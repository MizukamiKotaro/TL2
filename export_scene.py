...
class MYADDON_OT_export_scene(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    def parse_scene_recursive(self, file, object, level):
        ...
        #回転をQuaternion から Euler (3軸での回転)に変換
        rot = rot.to_euler()
        #ラジアンから度数法に変換
        ...
        #カスタムプロパティ 'player_spwan'
        if "player_spwan" in object:
            self.write_and_print(file, indent + "P %s" % object["player_spwan"])
        #カスタムプロパティ 'disabled'
        if "enemy_spwan" in object:
            self.write_and_print(file, indent + "E %s" % object["enemy_spwan"])
        #カスタムプロパティ'file_name'
    ...
    def parse_scene_recursive_json(self,data_parent,object,level):
        ...
        #オブジェクトのローカルトランスフォームから平行移動、回転、スケール
        trans,rot,scale = object.matrix_local.decompose()
        ...
        #カスタムプロパティ 'player_spwan'
        if "player_spwan" in object:
            json_object["player_spwan"] = object["player_spwan"]

        #カスタムプロパティ 'enemy_spwan'
        if "enemy_spwan" in object:
            json_object["enemy_spwan"] = object["enemy_spwan"]
